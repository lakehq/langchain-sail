...
from langchain_sail.prompts import QUERY_CHECKER
...
class QuerySailSQLTool(BaseSailSQLTool, BaseTool):
    ...
    description: str = (
        ...
        " output is a result from Sail SQL."
        " If the query is not correct, an error message will be returned."
        " If an error is returned, rewrite the query, check the query,"
        " and try again."
    )
...
class SailQueryCheckerTool(BaseSailSQLTool, BaseTool):
    ...
    def initialize_llm_chain(cls, values: Dict[str, Any]) -> Any:
        ...
        if values["llm_chain"].prompt.input_variables != ["query"]:
            msg = (
                "LLM chain for SailQueryCheckerTool need to use ['query']"
                " as input_variables for the embedded prompt"
            )
